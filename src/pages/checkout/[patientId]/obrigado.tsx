...
export default function ThankYou() {
  ...
  function handleChangeStar(starId: number) {
    ...
    setRateStart(rateStars.map((star, index) => index <= starIndex ? {
      id: star.id,
      isFilled: true
    } : {
      id: star.id,
  ...
  return (
    ...
    >
      <Content>
        ...
        <Button onClick={() => setDisplayRateModal(true)}>Avaliar experiência</Button>
        <Link href='/'>
          <a>Ir para o início</a>
        </Link>
      </Content>
      <Modal isOpen={displayRateModal} setIsOpen={() => setDisplayRateModal(false)}>
        <ModalContainer>
          <div>
          ...
          <div>
            ...
            <p>Nós valorizamos sua opinião e esperamos que você possa dedicar um minuto para responder algumas perguntas curtinhas que vão nos ajudar a entender como estamos indo.</p>
            <h3>Qual a probabilidade de você recomendar os serviços da Heali alguém da sua família ou amigo?</h3>
