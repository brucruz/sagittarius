...
import { MdLock, MdMail, MdPerson, MdVerifiedUser } from "react-icons/md";
...
const SignUpPage = () => {
  ...
  const [selectedIdType, setSelectedIdType] = useState<'RG' | 'CPF' | 'Passaporte' | 'RNE'>(null);
  ...
  const handleSubmit = useCallback(
    async (data: SigUpFormData) => {
      try {
        ...
        console.log(parse(birthDate, 'dd/MM/yyyy', new Date()));
        ...
        // const { data: patient} = await api.post<Patient>('/users', createPatientData);
  ...
  return (
    ...
    >
      <Form ref={formRef} onSubmit={handleSubmit}>
        ...
        <DateSelector name='birth_date' startDate={new Date} calendar={false} getTypedDate={handleGetBirthDate} label='Data de nascimento' />
        ...
        <RadioButtonGroup>
          {documentIdType && documentIdType.map(type => (
            <RadioButton isChecked={selectedIdType === type.id} label={type.label} name='sex' />
          ))}
        </RadioButtonGroup>
