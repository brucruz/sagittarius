...
const PageHeader = ({
  ...
  const handleBackButtonClick = useCallback(() => {
    buttonType.type === 'change_state_button' && buttonType.backButtonStateCallback(buttonType.backButtonNewState);
  }, [buttonType]);
  ...
  return (
    <PageHeaderContainer>
      {buttonType.type === 'change_state_button' && <ButtonBack onClick={handleBackButtonClick}/>}

      {buttonType.type === 'link' && <LinkBack url={buttonType.backLinkUrl}/>}

      {buttonType.type === 'change_state_button' && buttonType.stepper === '1/2' && (
        <img src={stepper1} alt="passo 1"/>
      )}

      {buttonType.type === 'go_back_button' && <ButtonBack onClick={handleGoBackButtonClick}/>}

      {buttonType.type === 'change_state_button' && buttonType.stepper === '2/2' && (
